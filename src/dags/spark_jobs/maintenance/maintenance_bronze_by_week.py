...
def expire_snapshots(spark, bronze_table, catalog, days=7, retain_last=5):
    try:
...
def remove_orphan_files(spark, bronze_table, catalog, days=3):
    try:
...
def run_maintenance_bronze_by_week():
    ...
    try:
        ...
        logger.info("-----> [MAINTENANCE-BRONZE-WEEKLY] Khởi tạo Spark Session...")
        ...
        bronze_table = f"{catalog}.{namespace}.{table_name}"
        
        logger.info(f"-----> [MAINTENANCE-BRONZE-WEEKLY] Bắt đầu maintenance cho bảng {bronze_table}")
        expire_snapshots(spark, bronze_table, catalog, days=7, retain_last=5)
        remove_orphan_files(spark, bronze_table, catalog, days=3)
