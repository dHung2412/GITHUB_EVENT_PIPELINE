...
from pyspark.sql.types import ArrayType, StringType, StructType, StructField, LongType, BooleanType, IntegerType
...
def get_spark_schema() -> StructType:    
    ...
    return StructType([
        ...
        StructField("actor", actor_fields, True),
        StructField("repo", repo_fields, True),
        StructField("payload", StringType(), True),
        StructField("public", BooleanType(), True),
        StructField("created_at", StringType(), True),
    ])

def get_bronze_projection() -> List[Column]:
    """Định nghĩa danh sách các cột sẽ được trích xuất (Flattening)"""
    return [
        col("data.id"),
        col("data.type"),
...
def write_batch_to_bronze(batch_df: DataFrame, batch_id: int, bronze_table: str):
    try:
        batch_df.writeTo(bronze_table) \
            ...
            .append()
        logger.info(f"-----> [BATCH {batch_id}] Successfully committed to Iceberg.")

    except Exception as e:
        logger.error(f"-----> Batch {batch_id}: Critical write error: {e}", exc_info=True)
        raise
...
def main():
    ...
    try:
        ...
        spark = SparkClient(app_name="kafka-to-bronze", job_type="streaming").get_session()
        
        # Thiết lập log level mặc định là WARN
        ...
        logger.info("-----> Stream is active. Waiting for data...")
        ...
        if spark:
            spark.stop()
        logger.info("-----> Cleanup complete. Shutdown successful.")
